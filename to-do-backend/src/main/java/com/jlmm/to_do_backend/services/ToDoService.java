...
public class ToDoService {
	public ResponseEntity<Object> createToDo(ToDo toDo){
		...
		try {
			...
			ToDoList.instance.increaseList(newToDo);
	...
	private Map<String, Object> getTimeIntervals(){
		...
		for(ToDo toDo : ToDoList.instance.getToDos()) {
			...
			if(toDo.isDone()) {
				...
				metrics.put("toDosAverageTime", Utils.milliecondsToMMSSFormat(toDosAverageTime));
				...
				if(toDo.getPriority() == Priority.HIGH) {
					...
					metrics.put("highToDosAverageTime", Utils.milliecondsToMMSSFormat(hightoDosAverageTime));
					...
					metrics.put("mediumToDosAverageTime", Utils.milliecondsToMMSSFormat(mediumToDosAverageTime));
					...
					metrics.put("lowToDosAverageTime", Utils.milliecondsToMMSSFormat(lowToDosAverageTime));
				} else {
